...
export const schedulePowerOn = (settingsData, side, power) => {
    if (!power.enabled)
        return;
...
const scheduleAnalyzeSleep = (offHour, offMinute, timeZone, side) => {
...
export const schedulePowerOffAndSleepAnalysis = (settingsData, side, power) => {
    if (!power.enabled)
        return;
    if (settingsData[side].awayMode)
        return;
